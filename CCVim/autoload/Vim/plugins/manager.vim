"=============================================================================
" manager.vim --- plugin manager for Vim
...
" Load Vim api
let s:VIM_CO = Vim#api#import('vim#compatible')
let s:JOB = Vim#api#import('job')
        ...
        if filereadable(expand(g:vim_plugin_bundle_dir)
            ...
            if s:need_cmd('git')
                call s:VIM_CO.system([
                            ...
                            \ expand(g:vim_plugin_bundle_dir) . 'neobundle.vim'
                            \ ])
                let g:vim_neobundle_installed = 1
            endif
        endif
...
function! s:need_cmd(cmd) abort
    if executable(a:cmd)
        ...
        echohl WarningMsg
        echom '[Vim] [plugin manager] You need install ' . a:cmd . '!'
        echohl None
        return 0
    endif
endfunction
...
function! Vim#plugins#manager#install(...) abort
    ...
    if empty(s:plugins)
        echohl WarningMsg
        echom '[Vim] Wrong plugin name, or all of the plugins are already installed.'
        echohl None
        return
    endif
    ...
    if status == 0
        echohl WarningMsg
        echom '[Vim] [plugin manager] plugin manager process is not finished.'
        echohl None
        return
    elseif status == 1
        " resume window
    ...
    for i in range(g:vim_plugin_manager_max_processes)
        if !empty(s:plugins)
...
" @vimlint(EVL102, 1, l:i)
function! Vim#plugins#manager#update(...) abort
    if !s:JOB.vim_job && !s:JOB.nvim_job
        let &maxfuncdepth = 2000
    endif
    let status = s:new_window()
    if status == 0
        echohl WarningMsg
        echom '[Vim] [plugin manager] plugin updating is not finished.'
        echohl None
        return
    elseif status == 1
        return
...
function! s:new_window() abort
    ...
    elseif s:plugin_manager_buffer != 0 && !bufexists(s:plugin_manager_buffer)
        ...
        execute get(g:, 'vim_window', 'vertical topleft new')
