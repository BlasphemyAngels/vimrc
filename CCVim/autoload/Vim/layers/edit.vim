...
let s:PASSWORD = Vim#api#import('password')
let s:NUMBER = Vim#api#import('data#number')
let s:LIST = Vim#api#import('data#list')

function! Vim#layers#edit#plugins() abort
    let plugins = [
                \ ['tpope/vim-surround'],
                \ ['junegunn/vim-emoji'],
...
function! Vim#layers#edit#config() abort
    ...
    let g:_vim_mappings_space.x = {'name' : '+Text'}
    let g:_vim_mappings_space.x.a = {'name' : '+align'}
    let g:_vim_mappings_space.x.d = {'name' : '+delete'}
    let g:_vim_mappings_space.x.i = {'name' : '+change symbol style'}
    ...
    call Vim#mapping#space#def('nmap', ['x', 'c'], '<Plug>CountSelectionRegion', 'cunt in the selection region', 0, 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'a', '&'], 'Tabularize /&', 'align region at &', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'a', '('], 'Tabularize /(', 'align region at (', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'a', ')'], 'Tabularize /)', 'align region at )', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'a', '['], 'Tabularize /[', 'align region at [', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'a', ']'], 'Tabularize /]', 'align region at ]', 1)
    ...
    call Vim#mapping#space#def('nnoremap', ['x', 'a', '='], 'Tabularize /=', 'align region at =', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'a', '¦'], 'Tabularize /¦', 'align region at ¦', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'd', 'w'], 'StripWhitespace', 'delete trailing whitespaces', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'd', '[SPC]'], 'silent call call('
                \ . string(s:_function('s:delete_extra_space')) . ', [])',
                \ 'delete extra space arround cursor', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'i', 'c'], 'silent call call('
    ...
    call Vim#mapping#space#def('nnoremap', ['x', 'i', '_'], 'silent call call('
                \ . string(s:_function('s:under_score')) . ', [])',
                \ 'change symbol style to under_score', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'i', 'u'], 'silent call call('
                \ . string(s:_function('s:under_score')) . ', [])',
                \ 'change symbol style to under_score', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'i', 'U'], 'silent call call('
                \ . string(s:_function('s:up_case')) . ', [])',
                \ 'change symbol style to UP_CACE', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'i', 'k'], 'silent call call('
                \ . string(s:_function('s:kebab_case')) . ', [])',
                \ 'change symbol style to kebab-case', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'i', '-'], 'silent call call('
                \ . string(s:_function('s:kebab_case')) . ', [])',
                \ 'change symbol style to kebab-case', 1)


    let g:_vim_mappings_space.i = {'name' : '+Insertion'}
    let g:_vim_mappings_space.i.l = {'name' : '+Lorem-ipsum'}
    let g:_vim_mappings_space.i.p = {'name' : '+Passwords'}
    let g:_vim_mappings_space.i.U = {'name' : '+UUID'}
    call Vim#mapping#space#def('nnoremap', ['i', 'p', 1], 'call call('
                \ . string(s:_function('s:insert_simple_password')) . ', [])',
                \ 'insert simple password', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'p', 2], 'call call('
                \ . string(s:_function('s:insert_stronger_password')) . ', [])',
                \ 'insert stronger password', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'p', 3], 'call call('
                \ . string(s:_function('s:insert_paranoid_password')) . ', [])',
                \ 'insert password for paranoids', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'p', 'p'], 'call call('
                \ . string(s:_function('s:insert_phonetically_password')) . ', [])',
                \ 'insert a phonetically easy password', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'p', 'n'], 'call call('
                \ . string(s:_function('s:insert_numerical_password')) . ', [])',
                \ 'insert a numerical password', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'u'], 'Unite unicode', 'search and insert unicode', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'U', 'U'], 'call call('
                \ . string(s:_function('s:uuidgen_U')) . ', [])',
                \ 'uuidgen-4', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'l', 'l'], 'call call('
                \ . string(s:_function('s:insert_lorem_ipsum_list')) . ', [])',
                \ 'insert lorem-ipsum list', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'l', 'p'], 'call call('
                \ . string(s:_function('s:insert_lorem_ipsum_paragraph')) . ', [])',
                \ 'insert lorem-ipsum paragraph', 1)
    call Vim#mapping#space#def('nnoremap', ['i', 'l', 's'], 'call call('
                \ . string(s:_function('s:insert_lorem_ipsum_sentence')) . ', [])',
                \ 'insert lorem-ipsum sentence', 1)
    let g:_vim_mappings_space.x.g = {'name' : '+translate'}
    call Vim#mapping#space#def('nnoremap', ['x', 'g', 't'], 'Ydc', 'translate current word', 1)

    " move line
    call Vim#mapping#space#def('nnoremap', ['x', 'J'], 'call call('
                \ . string(s:_function('s:move_text_down_transient_state')) . ', [])',
                \ 'move text down(enter transient state)', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 'K'], 'call call('
                \ . string(s:_function('s:move_text_up_transient_state')) . ', [])',
                \ 'move text up(enter transient state)', 1)

    " transpose
    let g:_vim_mappings_space.x.t = {'name' : '+transpose'}
    call Vim#mapping#space#def('nnoremap', ['x', 't', 'c'], 'call call('
                \ . string(s:_function('s:transpose_with_previous')) . ', ["character"])',
                \ 'swap current character with previous one', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 't', 'w'], 'call call('
                \ . string(s:_function('s:transpose_with_previous')) . ', ["word"])',
                \ 'swap current word with previous one', 1)
    call Vim#mapping#space#def('nnoremap', ['x', 't', 'l'], 'call call('
...
let s:STRING = Vim#api#import('data#string')
...
function! Vim#layers#edit#add_ft_head_tamplate(ft, tamp)
