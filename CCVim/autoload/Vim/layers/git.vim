function! Vim#layers#git#plugins() abort
  let plugins = [
...
function! Vim#layers#git#config() abort
  if has('patch-8.0.0027') || has('nvim')
    ...
    nnoremap <silent> <Leader>gA :Gina add .<CR>
    let g:_vim_mappings.g = {'name' : '+git function',
  ...
  augroup vim_layer_git
    autocmd!
    autocmd FileType diff nnoremap <buffer><silent> q :bd!<CR>
    autocmd FileType gitcommit setl omnifunc=Vim#plugins#gitcommit#complete
  augroup END
endfunction
