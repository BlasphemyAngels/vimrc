"autocmds
function! Vim#autocmds#init() abort
  augroup Vim_core
    au!
    ...
    autocmd QuitPre * call Vim#plugins#windowsmanager#UpdateRestoreWinInfo()
    autocmd WinEnter * call Vim#plugins#windowsmanager#MarkBaseWin()
    autocmd FileType html,css,jsp EmmetInstall
    autocmd BufRead,BufNewFile *.pp setfiletype puppet
    if g:vim_enable_cursorline == 1
      autocmd BufEnter,WinEnter,InsertLeave * setl cursorline
      autocmd BufLeave,WinLeave,InsertEnter * setl nocursorline
    endif
    if g:vim_enable_cursorcolumn == 1
    ...
    autocmd BufEnter,FileType * call Vim#mapping#space#refrashLSPC()
    if executable('synclient') && g:vim_auto_disable_touchpad
      let s:touchpadoff = 0
    ...
    autocmd VimEnter * call Vim#autocmds#VimEnter()
  augroup END
endfunction
...
function! s:generate_doc() abort
  if filereadable('./addon-info.json') && executable('vimdoc')
    call Vim#api#import('job').start(['vimdoc', '.'])
  endif
endfunction
...
function! s:fix_gruvbox() abort
  ...
  hi VimLeaderGuiderGroupName cterm=bold ctermfg=175 gui=bold guifg=#d3869b
endfunction
function! Vim#autocmds#VimEnter() abort
  call Vim#api#import('vim#highlight').hide_in_normal('EndOfBuffer')
  for argv in g:_vim_mappings_space_custom
    call call('Vim#mapping#space#def', argv)
  endfor
  if get(g:, '_vim_statusline_loaded', 0) == 1
    set laststatus=2
    call Vim#layers#core#statusline#def_colors()
    setlocal statusline=%!Vim#layers#core#statusline#get(1)
  endif
  if get(g:, '_vim_tabline_loaded', 0) == 1
    call Vim#layers#core#tabline#def_colors()
    set showtabline=2
  endif
endfunction
