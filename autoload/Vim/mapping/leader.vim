function! Vim#mapping#leader#defindglobalMappings() abort
  if g:vim_enable_insert_leader
  ...
  " Fast saving
  call Vim#mapping#def('nnoremap', '<Leader>w', ':w<CR>',
        \ 'Save current file',
        \ 'w',
        \ 'Save current file')
  call Vim#mapping#def('vnoremap', '<Leader>w', '<Esc>:w<CR>',
        \ 'Save current file',
        \ 'w',
        \ 'Save current file')

  let g:_vim_mappings.t = {'name' : 'Toggle editor visuals'}
  ...
  let g:_vim_mappings.l = {'name' : 'Location list movement'}
  call Vim#mapping#def('nnoremap', '<Leader>lj', ':lnext<CR>',
        \ 'Jump to next location list position',
        \ 'lnext',
        \ 'Next location list')
  call Vim#mapping#def('nnoremap', '<Leader>lk', ':lprev<CR>',
        \ 'Jump to previous location list position',
        \ 'lprev',
        \ 'Previous location list')
  call Vim#mapping#def('nnoremap', '<Leader>lq', ':lclose<CR>',
        \ 'Close the window showing the location list',
        \ 'lclose',
        \ 'Close location list window')

  " quickfix list movement
  let g:_vim_mappings.q = {'name' : 'Quickfix list movement'}
  call Vim#mapping#def('nnoremap', '<Leader>qj', ':cnext<CR>',
        \ 'Jump to next quickfix list position',
        \ 'cnext',
        \ 'Next quickfix list')
  call Vim#mapping#def('nnoremap', '<Leader>qk', ':cprev<CR>',
        \ 'Jump to previous quickfix list position',
        \ 'cprev',
        \ 'Previous quickfix list')
  call Vim#mapping#def('nnoremap', '<Leader>qq', ':cclose<CR>',
        \ 'Close quickfix list window',
        \ 'cclose',
        \ 'Close quickfix list window')
  call Vim#mapping#def('nnoremap <silent>', '<Leader>qr', 'q',
        \ 'Toggle recording',
        \ '',
  ...
  call Vim#mapping#def('nnoremap <silent>', '<Leader><C-c>',
  ...
  call Vim#mapping#def('nnoremap <silent>', '<Leader><C-l>',
  ...
  call Vim#mapping#def('vnoremap <silent>', '<Leader><C-l>',
  ...
  call Vim#mapping#def('vnoremap', '<Leader>S',
        \ "y:execute @@<CR>:echo 'Sourced selection.'<CR>",
        \ 'Sourced selection.',
        \ "echo 'Use <leader>S to sourced selection.'")
  call Vim#mapping#def('nnoremap', '<Leader>S',
        \ "^vg_y:execute @@<CR>:echo 'Sourced line.'<CR>",
        \ 'Source line',
        \ "echo 'Use <leader>S to sourced line.'")

  call Vim#mapping#def('nnoremap <silent>', '<Leader>sv',
        \ ':call Vim#mapping#split_previous_buffer()<CR>',
        \ 'Open previous buffer in split window',
        \ 'call Vim#mapping#split_previous_buffer()',
        \ 'Split previout buffer')
  call Vim#mapping#def('nnoremap <silent>', '<Leader>sg',
        \ ':call Vim#mapping#vertical_split_previous_buffer()<CR>',
        \ 'Open previous buffer in vsplit window' ,
        \ 'call Vim#mapping#vertical_split_previous_buffer()')
endfunction

function! Vim#mapping#leader#defindWindowsLeader(key) abort
  if !empty(a:key)
    ...
    call zvim#util#defineMap('nnoremap <silent>', '[Window]Q',
          \ ':<C-u>call Vim#mapping#close_current_buffer()<CR>',
          \ 'delete current windows',
          \ 'call Vim#mapping#close_current_buffer()')
    call zvim#util#defineMap('nnoremap <silent>', '[Window]c',
          \ ':<C-u>call Vim#mapping#clearBuffers()<CR>',
          \ 'Clear all the buffers', 'call Vim#mapping#clearBuffers()')
  endif
endfunction

function! Vim#mapping#leader#defindDeniteLeader(key) abort
  if !empty(a:key)
    if a:key == 'F'
      nnoremap <leader>F F
    endif
    ...
    let g:_vim_mappings_denite = {}
    nnoremap <silent> [denite]r
          \ :<C-u>Denite -resume<CR>
    let g:_vim_mappings_denite.r = ['Denite -resume',
          \ 'resume denite window']
    nnoremap <silent> [denite]f  :<C-u>Denite file_rec<cr>
    let g:_vim_mappings_denite.f = ['Denite file_rec', 'file_rec']
    nnoremap <silent> [denite]g  :<C-u>Denite grep<cr>
    let g:_vim_mappings_denite.g = ['Denite grep', 'denite grep']
    nnoremap <silent> [denite]j  :<C-u>Denite jump<CR>
    let g:_vim_mappings_denite.j = ['Denite jump', 'denite jump']
    nnoremap <silent> [denite]<C-h>  :<C-u>DeniteCursorWord help<CR>
    let g:_vim_mappings_denite['<C-h>'] = ['DeniteCursorWord help',
          \ 'denite with cursor word help']
    nnoremap <silent> [denite]o  :<C-u>Denite -buffer-name=outline
          \  -auto-preview outline<CR>
    let g:_vim_mappings_denite.o = ['Denite outline', 'denite outline']
    nnoremap <silent> [denite]e  :<C-u>Denite
          \ -buffer-name=register register<CR>
    let g:_vim_mappings_denite.e = ['Denite register', 'denite register']
    nnoremap <silent> [denite]<Space> :Denite menu:CustomKeyMaps<CR>
    let g:_vim_mappings_denite['<space>'] = ['Denite menu:CustomKeyMaps',
          \ 'denite customkeymaps']
  endif
endfunction

function! Vim#mapping#leader#defindUniteLeader(key) abort
  if !empty(a:key)
    if a:key == 'f'
      nnoremap <leader>f f
    endif
    ...
    let g:_vim_mappings_unite = {}
    nnoremap <silent> [unite]r
          \ :<C-u>Unite -buffer-name=resume resume<CR>
    let g:_vim_mappings_unite.r = ['Unite -buffer-name=resume resume',
          \ 'resume unite window']
    if has('nvim')
      nnoremap <silent> [unite]f  :<C-u>Unite file_rec/neovim<cr>
      let g:_vim_mappings_unite.f = ['Unite file_rec/neovim', 'file_rec']
    else
      nnoremap <silent> [unite]f  :<C-u>Unite file_rec/async<cr>
      let g:_vim_mappings_unite.f = ['Unite file_rec/async', 'file_rec']
    endif
    nnoremap <silent> [unite]i  :<C-u>Unite file_rec/git<cr>
    let g:_vim_mappings_unite.i = ['Unite file_rec/git', 'git files']
    nnoremap <silent> [unite]g  :<C-u>Unite grep<cr>
    let g:_vim_mappings_unite.g = ['Unite grep', 'unite grep']
    nnoremap <silent> [unite]u  :<C-u>Unite source<CR>
    let g:_vim_mappings_unite.u = ['Unite source', 'unite source']
    nnoremap <silent> [unite]t  :<C-u>Unite tag<CR>
    let g:_vim_mappings_unite.t = ['Unite tag', 'unite tag']
    nnoremap <silent> [unite]T  :<C-u>Unite tag/include<CR>
    let g:_vim_mappings_unite.T = ['Unite tag/include',
          \ 'unite tag/include']
    nnoremap <silent> [unite]l  :<C-u>Unite locationlist<CR>
    let g:_vim_mappings_unite.l = ['Unite locationlist',
          \ 'unite locationlist']
    nnoremap <silent> [unite]q  :<C-u>Unite quickfix<CR>
    let g:_vim_mappings_unite.q = ['Unite quickfix', 'unite quickfix']
    nnoremap <silent> [unite]e  :<C-u>Unite
          \ -buffer-name=register register<CR>
    let g:_vim_mappings_unite.e = ['Unite register', 'unite register']
    nnoremap <silent> [unite]j  :<C-u>Unite jump<CR>
    let g:_vim_mappings_unite.j = ['Unite jump', 'unite jump']
    nnoremap <silent> [unite]h  :<C-u>Unite history/yank<CR>
    let g:_vim_mappings_unite.h = ['Unite history/yank',
          \ 'unite history/yank']
    nnoremap <silent> [unite]<C-h>  :<C-u>UniteWithCursorWord help<CR>
    let g:_vim_mappings_unite['<C-h>'] = ['UniteWithCursorWord help',
          \ 'unite with cursor word help']
    nnoremap <silent> [unite]s  :<C-u>Unite session<CR>
    let g:_vim_mappings_unite.s = ['Unite session', 'unite session']
    nnoremap <silent> [unite]o  :<C-u>Unite -buffer-name=outline
          \ -start-insert -auto-preview -split outline<CR>
    let g:_vim_mappings_unite.o = ['Unite outline', 'unite outline']
    ...
    let g:_vim_mappings_unite.m = {'name' : 'unite menus',
    ...
    let g:_vim_mappings_unite['[SPC]'] = ['Unite -silent -ignorecase' .
          \ ' -winheight=17 -start-insert menu:CustomKeyMaps',
          \ 'unite customkeymaps']
  endif
endfunction

function! Vim#mapping#leader#getName(key) abort
  if a:key == g:vim_unite_leader
    return '[unite]'
  elseif a:key == g:vim_denite_leader
    return '[denite]'
  elseif a:key == ' '
    return '[SPC]'
  elseif a:key == 'g'
...
function! Vim#mapping#leader#defindKEYs() abort
  let g:_vim_mappings_prefixs = {}
  let g:_vim_mappings_prefixs[g:vim_unite_leader] = {'name' : '+Unite prefix'}
  call extend(g:_vim_mappings_prefixs[g:vim_unite_leader], g:_vim_mappings_unite)
  let g:_vim_mappings_prefixs[g:vim_denite_leader] = {'name' : '+Denite prefix'}
  call extend(g:_vim_mappings_prefixs[g:vim_denite_leader], g:_vim_mappings_denite)
endfunction
