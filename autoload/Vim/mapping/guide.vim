...
function! Vim#mapping#guide#has_configuration() "{{{
  return exists('s:desc_lookup')
endfunction "}}}

function! Vim#mapping#guide#register_prefix_descriptions(key, dictname) " {{{
...
function! Vim#mapping#guide#populate_dictionary(key, dictname) " {{{
  call s:start_parser(a:key, s:cached_dicts[a:key])
endfunction " }}}
" @vimlint(EVL103, 0, a:dictname)

function! Vim#mapping#guide#parse_mappings() " {{{
...
function! s:updateStatusline() abort
  call Vim#mapping#guide#theme#hi()
  ...
  exe 'setlocal statusline=%#LeaderGuiderPrompt#\ Guide:\ ' .
        ...
        \ '%#LeaderGuiderName#\ ' .
        \ Vim#mapping#leader#getName(s:prefix_key)
...
function! Vim#mapping#guide#start_by_prefix(vis, key) " {{{
  if a:key == ' ' && exists('b:vim_lang_specified_mappings')
    let g:_vim_mappings_space.l = b:vim_lang_specified_mappings
  endif
  let s:guide_help_mode = 0
  let s:vis = a:vis ? 'gv' : ''
...
function! Vim#mapping#guide#start(vis, dict) " {{{
...
if get(g:, 'mapleader', '\') == ' '
  call Vim#mapping#guide#register_prefix_descriptions(' ',
        \ 'g:_vim_mappings')
else
  call Vim#mapping#guide#register_prefix_descriptions('\',
        \ 'g:_vim_mappings')
  call Vim#mapping#guide#register_prefix_descriptions(' ',
        \ 'g:_vim_mappings_space')
  call Vim#plugins#help#regist_root({'SPC' : g:_vim_mappings_space})
endif
call Vim#mapping#guide#register_prefix_descriptions(
      \ g:vim_unite_leader,
      \ 'g:_vim_mappings_unite')
call Vim#mapping#guide#register_prefix_descriptions(
      \ g:vim_denite_leader,
      \ 'g:_vim_mappings_denite')
call Vim#mapping#guide#register_prefix_descriptions(
      \ '[KEYs]',
      \ 'g:_vim_mappings_prefixs')
call Vim#mapping#guide#register_prefix_descriptions(
      \ 'g',
      \ 'g:_vim_mappings_g')
call Vim#mapping#guide#register_prefix_descriptions(
      \ 'z',
      \ 'g:_vim_mappings_z')
let [s:lsep, s:rsep] = Vim#layers#core#statusline#rsep()
let &cpo = s:save_cpo
unlet s:save_cpo
